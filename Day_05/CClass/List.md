...
- ```list.sort(func())```
- ```(l2,sum(l2))```
- ```list.clear``` - Removes all the elements in the list, just like ```del [a:]```
...
# constant iterators
- ContinuousIterator is a simple iterator class that starts from a given value (start) and increments continuously. It overrides the necessary operators (*, ++, ==, !=) to make it usable in range-based loops.
